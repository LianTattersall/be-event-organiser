...
import { deleteExternalSaved, getExternalSavedById, getExternalSavedEvents, postExternalSaved } from '../controllers/external-controllers';

const external_routes = new Hono<{ Bindings: Env }>();

external_routes.get('/:user_id', getExternalSavedEvents);

external_routes.get('/:user_id/:event_id', getExternalSavedById);
